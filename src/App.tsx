...
const App: React.FC = () => {
    ...
    const handleSelectKey = async () => {
        if (window.aistudio && window.aistudio.openSelectKey) {
            ...
            const key = prompt(t('enterApiKey') || 'Please enter your Gemini API Key:');
            if (key && key.trim() !== '') {
        ...
        return (
            <div className="min-h-screen bg-[#020617] flex flex-col items-center justify-center p-6 relative overflow-hidden">
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10 max-w-md w-full bg-slate-900/40 backdrop-blur-2xl border border-slate-800 rounded-3xl p-12 shadow-2xl text-center">
                    ...
                    <button onClick={handleSelectKey} className="w-full bg-white hover:bg-slate-200 text-slate-950 font-bold text-lg py-4 rounded-xl transition-all shadow-lg transform active:scale-95 cursor-pointer">
                        {t('authenticate')}
                    </button>
                    <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="inline-block mt-4 text-emerald-400 hover:text-emerald-300 underline text-sm transition-colors">
